...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{
			desc: "waverouter model, expect defaults",
			nImpl: &node.Impl{
				Proto: &tpb.Node{
					Model: ModelWR,
				},
			...
			want: &tpb.Node{
				...
				Config: &tpb.Config{
					ConfigPath: "config/",
					ConfigFile: "config.cfg",
					Image:      "artifactory.ciena.com/psa/rw-containerlab:latest",
					Env: map[string]string{
						"CLAB_LABEL_CLAB_NODE_NAME": "",
						"CLAB_LABEL_CLAB_NODE_TYPE": ModelWR,
					},
				},
				Services: map[uint32]*tpb.Service{
					22:    {Names: []string{"ssh"}, Inside: 22},
...
func TestNode_CreatePod_ConfigData(t *testing.T) {
	nodeProto := &tpb.Node{
		Name: "wr-cfg",
		Config: &tpb.Config{
			Image:      "vrnetlab/ciena_waverouter:config",
			...
			ConfigData: &tpb.Config_Data{
				Data: []byte("hostname ciena-router"),
			},
		},
	}
	n := &Node{
